      ...
      <CardContent className="grid gap-4">
        <form onSubmit={onSubmit} className="grid gap-4">
          ...
          <div className="text-xs text-muted-foreground">
            Попередній перегляд повного URL: {url}{queryString ? `?${queryString}` : ""}
          </div>

          <Button type="submit" disabled={loading}>{loading ? "Аналіз..." : "Надіслати на аналіз ШІ"}</Button>
        </form>
      </CardContent>
    </Card>
  )
